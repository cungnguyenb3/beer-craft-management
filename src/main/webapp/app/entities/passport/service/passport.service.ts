...
import { IPassport, getPassportIdentifier } from '../passport.model';
...
export class PassportService {
  ...
  protected convertDateFromClient(passport: IPassport): IPassport {
    return Object.assign({}, passport, {
      additionTime: passport.additionTime?.isValid() ? passport.additionTime.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.additionTime = res.body.additionTime ? dayjs(res.body.additionTime) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((passport: IPassport) => {
        passport.additionTime = passport.additionTime ? dayjs(passport.additionTime) : undefined;
      });
    }
    return res;
  }
